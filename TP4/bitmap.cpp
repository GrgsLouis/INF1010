...
namespace
{
    std::vector<unsigned char> get_4_bytes(const uint32_t & data)
    {
    ...
    void insert_4_bytes(std::vector<unsigned char> & dest, const uint32_t & data)
    {
        std::vector<unsigned char> separated_data = get_4_bytes(data);
        std::copy(separated_data.begin(), separated_data.end(), back_inserter(dest));
    }
    std::vector<unsigned char> get_2_bytes(const uint16_t & data)
    {
        std::vector<unsigned char> ret;
        unsigned char* tmp = (unsigned char*)&data;
        ret.push_back(tmp[0]);
    ...
    void insert_2_bytes(std::vector<unsigned char> & dest, const uint16_t & data)
    {
        std::vector<unsigned char> separated_data = get_2_bytes(data);
 ...
 */
size_t bitmap_encode_rgb(const unsigned char* rgb, unsigned int width, unsigned int height, unsigned char** output)
{
    ...
    *output = new unsigned char[data.size()];
